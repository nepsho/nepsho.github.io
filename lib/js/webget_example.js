...
function createMetaTableContent(metaObj){
    ...
    for(var i = 0 ; i < metaFields.length ; i++){
      ...
      if(metaFields[i] == "logo" && metaObj["logo"] && metaObj["logo"].trim().length!=0){
        $("#output_logo_container").css("background-image","url("+metaObj['logo']+")");
      }
    }
...
function getHtml(searchUrl){
    ...
    $.ajax({
      type: "POST",
      url: "https://webgetdemo.herokuapp.com/get_html",
      data:{url:searchUrl},
...
function getMeta(searchUrl){
    ...
    $.ajax({
      type: "POST",
      url: "https://webgetdemo.herokuapp.com/get_meta",
      data:{url:searchUrl},
      success: function(data){
        if(!data){
...
$.ajax({
      type: "GET",
      url: "https://webgetdemo.herokuapp.com/",
      success: function(data){
        server_is_live(true);
      },
      error: function (textStatus, errorThrown) {
